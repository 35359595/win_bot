...
function getPlayerMove(data) {
    ...
    return Math.atan2(
        ...
        + ((Math.abs(currentPlayer.y - ball.y) > ballRadius * 2.1) 
            ...
            : ballStop.y < currentPlayer.y //otherwise need some extra space
                ? + ballRadius * 2
                : - ballRadius * 2),
        ballStop.x < ballRadius ? ballRadius : ballStop.x - currentPlayer.x + ( Math.abs(ballStop.y - currentPlayer.y) < ballRadius * 2 ? - ballRadius * 2.2 : - ballRadius * 2.02)
        );
    };
    ...
    let decide = () => {
        ...
        }else if(ballStop.x < wField / 3 && ballStop.x < currentPlayer.x + ballRadius * 3 && currentPlayer.x > secondPlayer.x || currentPlayer.x > thirdPlayer.x){
