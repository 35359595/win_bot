//11.12 - simplicity VIII-EX-3(place 6)
'use strict';
    ...
    return Math.atan2(
        ...
        + ((Math.abs(currentPlayer.y - ball.y) > ballRadius * 2) 
            ...
            : ballStop.y < currentPlayer.y //otherwise need some extra space
                ? + ballRadius * 2 - (amICloser() ? 0 : hField / 4)
                : - ballRadius * 2 + (amICloser() ? 0 : hField / 4)),
        ballStop.x < ballRadius ? ballRadius : ballStop.x - currentPlayer.x + (ball.speed == 0 ? 0 : - (ball.x - ballStop.x)) - ballRadius * 2.2
        );
    };
