...
function getPlayerMove(data) {
    ...
    let haveForward = () => {
        if(firstEnemy.x < ball.x || secondEnemy.x < ball.x || thirdEnemy.x < ball.x){
            return true;
        }return false;
    };
    ...
    var defend = () => {
    return Math.atan2(
        ballStop.y - currentPlayer.y + (Math.abs(ball.y - currentPlayer.y) < ballRadius * 2 ? ball.y - currentPlayer.y : 0),
        ballStop.x - currentPlayer.x - ballRadius * 2.5);//+ (ball.speed == 0 ? - ballRadius * 2 : - (ball.x - ballStop.x) - ballRadius * 2.2));
    };
    ...
    let decide = () => {
        if(data.playerIndex == 2 && ballStop.x == wField / 2 && ballStop.y == hField / 2){
            ...
            attackDirection = getDirectionTo(currentPlayer, zonePoint);
        }else if (!haveForward() || ball.x >= currentPlayer.x + ballRadius * 1.99){
