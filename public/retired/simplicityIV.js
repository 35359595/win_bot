    ...
    return Math.atan2(
        ...
        + ((Math.abs(currentPlayer.y - ball.y) > ballRadius * 2) 
            ...
            : ballStop.y < currentPlayer.y //otherwise need some extra space
                ? + ballRadius * 2
                : - ballRadius * 2),
        ballStop.x < ballRadius ? ballRadius : currentPlayer.x - ballStop.x - ballRadius * 2.2
        );
    };

    let decide = () => {
        if (ballStop.x >= currentPlayer.x + ballRadius * 2.2){
            attackDirection = attack();
        }else if(ballStop.x > wField / 2 && !whoIsCloser(currentPlayer, secondPlayer) && !whoIsCloser(currentPlayer, thirdPlayer)){
            attackDirection = homecoming(ballStop.x - wField / 3);
        }else{
