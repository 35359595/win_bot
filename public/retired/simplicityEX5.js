...
function getPlayerMove(data) {
    ...
    var defend = () => {
    return Math.atan2(
        ballStop.y - currentPlayer.y + (Math.abs(ball.y - currentPlayer.y) < ballRadius * 2 ? (ball.y < currentPlayer.y ? (currentPlayer.y - ballStop.y) * 2 : - (ballStop.y - currentPlayer.y) * 2) : ball.y < currentPlayer.y ? ballRadius : - ballRadius),
        ballStop.x - currentPlayer.x - ballRadius * 1.99);;
    };
    ...
    let decide = () => {
        if(data.playerIndex == 2 && ballStop.x == wField / 2 && ballStop.y == hField / 2){
            ...
            attackDirection = getDirectionTo(currentPlayer, zonePoint);
        }else if (ball.x >= currentPlayer.x + ballRadius * 1.99){
